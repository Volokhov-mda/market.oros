...
const SortingOption = ({
  ...
  const { onChange: onChangeOrderBy, ...regOrderByParams } =
    register("orderby");
  const { onChange: onChangeOrder, ...regOrderParams } = register("order");

  const onSelectOption = (isAscending) => {
  ...
  return (
    <>
      ...
      />
      <label
        for={orderable ? `${option.value}asc` : option.value}
      ...
      >
        ...
        {orderable && (
          <div className={styles.order}>
            <label
              for={`${option.value}asc`}
              className={clsx(
                isAscending && styles.selectedOption,
                styles.ascending
              )}
            >
              <input
