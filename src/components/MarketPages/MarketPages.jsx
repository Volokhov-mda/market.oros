...
const MarketPages = ({ currPage, setCurrPage, setTotalNumOfPages, usersPerPage, influencersCount }) => {
    ...
    const renderNumsOfPages = (numOfPages) => {
        if (numOfPages > 0) {
            const nums = [
                <button key={-2} className={clsx(styles.prevPage, styles.chevrons)} onClick={() => setNewPage(0)}><img className={styles.icon} src={chevrons} alt="left" /></button>,
                <button key={-1} className={styles.prevPage} onClick={() => setNewPage(currPage - 1)}><img className={styles.icon} src={chevron} alt="left" /></button>,
            ];

            if (numOfPages <= numOfPagesShown) {
                for (let i = 0; i < numOfPages; i++) {
                    nums.push(<MarketPageButton key={i} pageNum={i + 1} isActive={currPage === i} onClick={() => setNewPage(i)} />)
                }
                ...
                nums.push(<>
                    <span>...</span>
                    <MarketPageButton key={numOfPages - 1} pageNum={numOfPages} isActive={currPage === numOfPages - 1} onClick={() => setNewPage(numOfPages - 1)} />
                </>);
            } else if (currPage >= numOfPages - (Math.floor(numOfPagesShown / 2) + 1)) {
                for (let i = numOfPages - numOfPagesShown; i < numOfPages; i++) {
                    nums.push(<MarketPageButton key={i} pageNum={i + 1} isActive={currPage === i} onClick={() => setNewPage(i)} />)
                }
            } else {
                const upperLimit = (currPage + 3 < numOfPages - 1) ? currPage + (Math.floor(numOfPagesShown / 2) - 1) : numOfPages - (Math.floor(numOfPagesShown / 2) - 2);

                for (let i = currPage - (Math.floor(numOfPagesShown / 2) - 1); i <= upperLimit; i++) {
                    nums.push(<MarketPageButton key={i} pageNum={i + 1} isActive={currPage === i} onClick={() => setNewPage(i)} />)
                }
            ...
            nums.push([
                <button key={numOfPages} className={styles.nextPage} onClick={() => setNewPage(currPage + 1)}><img className={styles.icon} src={chevron} alt="left" /></button>,
                <button key={numOfPages + 1} className={clsx(styles.nextPage, styles.chevrons)} onClick={() => setNewPage(numOfPages - 1)}><img className={styles.icon} src={chevrons} alt="left" /></button>,
            ]);
